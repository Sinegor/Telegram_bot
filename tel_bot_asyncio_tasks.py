...
from async_script_tasks import set_starting_data, subscribe, string_handling, get_previous_week_btc_data_price_1
...
# # Обработчик history, но через класс Text:

# @sin_disp.message_handler(Text(equals='/history', ignore_case=True))
# async def history_handler(message):
#     user_id = message.from_id
#     user_name = message.from_user.full_name
#     logging.info(f'{time.asctime()}: start work whith user {user_id} {user_name}')
...
async def st_handler_1 (message: types.Message):
    ...
    try: 
        data = await set_starting_data(coin)
    except KeyError:
        await message.reply('Нет такой монеты!')
    await message.reply(f'Last week data on net percentage movement of {coin} asset compared to bitcoin movement: {data}')
